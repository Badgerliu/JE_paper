...
cd ./shift # Now you will be in ./shift folder
...
for foo in *_m_d_shifted.bed
	do
		...
		echo $bar  
		#peak calling
		macs2 callpeak --nomodel -t ${bar}_m_d_shifted.bed -n ${bar} --nolambda --gsize 1.87e9 --keep-dup all --slocal 10000
...
rm *_m_d_shifted.bed  # (optional) remove the converted bedfiles just to save disk space.
