...
load(file = "./Figure3_scRNA_part/Figure3_scRNA_part.Rdata")
...
ggsave(filename = "Figure3_scRNA_part/splom_dm_JE_combined_epi_sce_dm.png")
```
...
rglwidget()
rgl.snapshot('./Figure3_scRNA_part/Figure3B_3dplot.png', fmt = "png") # this is not working. Thus, after adjusting the angles, I save the rgl plot into PNG, and modifed in Adobe illustrator.
```


#3 Figure3C Krt14 and Krt5 smooth plot
...
ggsave("./Figure3_scRNA_part/Plot_Krt14_lineage1vs3.pdf",  width = 15, height = 5)
```
...
plotSmoothers(sce.trade, counts, "Krt5", curvesCols = curvesCols,size=1/10,
              border = FALSE) + xlim(0,15) +ylim(0,6)+
  scale_color_manual(values = curvesCols)
ggsave("./Figure3_scRNA_part/Plot_Krt5_lineage1vs3.pdf",  width = 15, height = 5)
```
#4 Summary of JE_K5
...
# combine
plotc <- plot1 + plot2
ggsave("./Figure3_scRNA_part/JE_K5_MAGIC_umap.pdf", plot = plotc, width = 20, height = 10)
plot3
ggsave("./Figure3_scRNA_part/JE_K5_MAGIC_umap_split.pdf", plot = plot3, width = 24, height = 10)
...
# Save the dot plot
ggsave("./Figure3_scRNA_part/JE_K5_harmony_dotplot_feature.pdf", width = 8.5, height = 5)
```
...
write.csv(split_count_melt, file = "./Figure3_scRNA_part/JE_K5_MAGIC_harmony_cell_number_celltype.csv", row.names = FALSE)
...
ggsave("./Figure3_scRNA_part/JE_K5_MAGIC_harmony_Percentage_cell_in_each_celltype_unnormalized.pdf", height = 3, width = 7.5)
...
write.csv(split_count_percent_melt, file = "./Figure3_scRNA_part/JE_K5_MAGIC_harmony_cell_percentage_celltype.csv", row.names = FALSE)
...
ggsave("./Figure3_scRNA_part/JE_K5_MAGIC_harmony_Percentage_cell_in_each_celltype_normalized.pdf", height = 3, width = 7.5)
...
ggsave("./Figure3_scRNA_part/JE_K5_MAGIC_harmony_Percentage_cell_in_each_celltype_normalized_stack.pdf", height = 3, width = 7.5)
...
ggplot(split_count_percent_melt, aes(x = celltype, y = Percentage)) +
  ...
  theme_bw() + 
  theme(panel.grid = element_blank(),axis.text.x = element_text(angle = 45, hjust = 0.5, vjust = 0.5))
ggsave("./Figure3_scRNA_part/JE_K5_MAGIC_harmony_Percentage_cell_in_each_celltype_normalized_dodge.pdf", height = 3, width = 7.5)
...
ggsave("./Figure3_scRNA_part/JE_K5_MAGIC_harmony_heatmap_cell_proportion.pdf", height = 5, width = 7)
...
FeaturePlot(JE_K5, 
                  ...
                  max.cutoff =5,
            label=T,repel = T)
ggsave("./Figure3_scRNA_part/JE_K5_GFP_Lor_Odam.pdf")
```
...
FeaturePlot(JE_K5_MAGIC, 
                  ...
                  max.cutoff =5,
            label=T,repel = T)
ggsave("./Figure3_scRNA_part/JE_K5_MAGIC_GFP_Lor_Odam.pdf")
```
...
FeaturePlot(JE_K5_MAGIC, 
                  ...
                  max.cutoff =5,
            label=T,repel = T)
ggsave("./Figure3_scRNA_part/JE_K5_MAGIC_GFP_Krt5_Lor_Odam_split_1.pdf")
```
```{r,GFP lor odam split MAGIC,fig.width=25,fig.height=27}
DefaultAssay(JE_K5_MAGIC) <- "MAGIC_SCT"
FeaturePlot(JE_K5_MAGIC, 
                  ...
                  max.cutoff =5,
            label=T,repel = T)
ggsave("./Figure3_scRNA_part/JE_K5_MAGIC_GFP_Krt5_Lor_Odam_split_0.1.pdf")
```
...
VlnPlot(JE_K5_epi_D0_D5, 
        ...
        cols = custom_col)
ggsave("./Figure3_scRNA_part/JE_K5_epi_D0_D5_GFP_vlnplot.pdf", height = 4, width = 7)
```
...
save(JE_combined, #seurat object fro combined with celltype information
     ...
     file = "./Figure3_scRNA_part/Figure3_scRNA_part.Rdata")
```
