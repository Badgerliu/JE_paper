...
load(file = "./Figure6_scRNA_part/Figure6_scRNA_part.Rdata")
...
write.csv(top10_earlyDERes, file = "./Figure6_scRNA_part/JE_combined_epi_harmony_SCT_renamed_slingshot_top_10_EarlyDriver_genes_with_different_pattern_among_all_lineages_20220822.csv", row.names = FALSE )
...
```{r,list top10 genes}
write.csv(topEDGenes_1v1, file = "./Figure6_scRNA_part/JE_combined_epi_harmony_SCT_renamed_slingshot_top_10_early_driver_genes_between_two_lineages_20220822.csv", row.names = FALSE )
```
...
heatSmooth <- pheatmap(t(scale(t(yhatSmooth_false[, c(100:1,201:300)]))), # yhatSmooth_false we used nPoints=100, so we choose 1:100 and 201:300 for lineage 1 and 3 respectively
                       ...
                       show_colnames = TRUE,
                       border_color = FALSE)
ggsave("./Figure6_scRNA_part/JE_combined_epi_harmony_SCT_renamed_slingshot_tradeSeq_lineage_1_vs3_bifurcation_heatmap.pdf", plot=heatSmooth, width = 10, height = 25)
```
...
# Save the plot
ggsave("./Figure6_scRNA_part/JE_combined_epi_harmony_SCT_renamed_slingshot_tradeSeq_lineage_1_vs3_bifurcation_heatmap_p_test.pdf", plot=heatSmooth, width = 10, height = 25)
...
## export the results
write.table(newOrder, "./Figure6_scRNA_part/JE_combined_epi_SCT_renamed_slingshot_tradeseq_1vs3_DEarly_bifurcation.csv", quote=F, row.names =T, col.names =T, sep ='\t')
```
...
for(cluster in 1:6) {
    ...
    print(plot)
    # Save the plot
    file_name <- paste0("./Figure6_scRNA_part/JE_epi_harmony_imputation_slingshot_ED_bifurcation_lineage_1_vs_3_cluster", cluster, ".pdf")
...
load(file = "./Figure6_scRNA_part/Figure6_scRNA_part.Rdata")
module_gene <- read.csv("./Figure6_scRNA_part/JE_combined_epi_SCT_renamed_slingshot_tradeseq_1vs3_DEarly_bifurcation.csv",sep="\t")
```
...
write.csv(Module_GO, file = './Figure6_scRNA_part/Module_GO.csv')
...
plotSmoothers(sce.trade, counts, "Trp63", curvesCols = curvesCols,size=1/10,
  ...
  scale_color_manual(values = curvesCols)
ggsave("./Figure6_scRNA_part/Plot_Trp63_lineage1vs3.pdf",  width = 15, height = 5)
```
...
plotSmoothers(sce.trade, counts, "Runx1", curvesCols = curvesCols,size=1/10,
              border = FALSE) +  xlim(0,15) +ylim(0,6)+
  scale_color_manual(values = curvesCols)
ggsave("./Figure6_scRNA_part/Plot_Runx1_lineage1vs3.pdf",  width = 15, height = 5)
```
...
ggsave("./Figure6_scRNA_part/JE_K5_epi_Trp63_umap.pdf")
```
...
ggsave("./Figure6_scRNA_part/JE_K5_epi_Klf4_umap.pdf")
```
...
ggsave("./Figure6_scRNA_part/JE_K5_epi_Itga3_umap.pdf")
```
...
ggsave("./Figure6_scRNA_part/JE_K5_epi_Itgb1_umap.pdf")
```
...
ggsave("./Figure6_scRNA_part/JE_K5_epi_Runx1_umap.pdf")
```
...
save(sce_dm, #sce in diffusion map
     ...
     file = "./Figure6_scRNA_part/Figure6_scRNA_part.Rdata")
```
